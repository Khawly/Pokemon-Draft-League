-- `save_priority_list` now preserves per-round auto/skip flags.
--
-- The arena auto-saves every priority-list edit, so the previous rewrite
-- (DELETE + INSERT without the flag columns) would silently reset the Auto
-- Pick / Skip Pick toggles on each change. This redefines the RPC to snapshot
-- each round's flags before the delete and re-apply them on insert.
