...
import detailsJson from "@/data/pokemon-details.json";
import abilitiesJson from "@/data/pokemon-abilities.json";
...
/** Per-stat base stat values for a species/form, keyed by stat kind. */
...
/** A single ability of a species/form, with its hidden-ability flag. */
export type PokemonAbility = {
  /** PokeAPI ability name, e.g. "lightning-rod". */
  name: string;
  /** True for the hidden (dream) ability; normal abilities are false. */
  hidden: boolean;
};

/** One entry of the bundled stats/abilities dataset, keyed by species slug. */
export type PokemonDetailsEntry = {
  /** PokeAPI slug, matching the pool/roster pokemon_id. */
  slug: string;
  /** The six base stats. */
  stats: PokemonBaseStats;
  /** Ability list (normal + hidden). */
...
/** The full stats/abilities dataset as a typed array. */
export const POKEMON_DETAILS = detailsJson as PokemonDetailsEntry[];

/** Index of the stats/abilities dataset by species slug. */
const DETAILS_BY_SLUG = new Map(
  POKEMON_DETAILS.map((entry) => [entry.slug, entry]),
);

/**
 * Returns the bundled base-stats/abilities entry for a species slug or form
 * slug, or null when the slug is unknown.
 *
 * @param slug - The PokeAPI slug, e.g. "pikachu" or "deoxys-defense".
 * @returns The stats/abilities entry, or null if the slug is unknown.
 */
export function getPokemonDetailsBySlug(
  slug: string,
): PokemonDetailsEntry | null {
  return DETAILS_BY_SLUG.get(slug) ?? null;
}

/** One record of the bundled abilities dataset. */
export type PokemonAbilityDescription = {
  /** PokeAPI ability resource name, e.g. "lightning-rod". */
  slug: string;
  /** Short English effect text, e.g. "Protects the bearer from Electric moves." */
  description: string | null;
};

/**
 * The bundled abilities dataset as a typed array.
 *
 * Generated by scripts/generate-pokemon-abilities.mjs from Serebii.net's
 * AbilityDex (each ability's In-Depth Effect text), so the app can explain the
 * full mechanics of an ability without a runtime network call.
 */
export const ABILITY_DATA = abilitiesJson as PokemonAbilityDescription[];

/** Index of ability descriptions by ability slug. */
const ABILITY_DESCRIPTION_BY_SLUG = new Map(
  ABILITY_DATA.map((entry) => [entry.slug, entry.description]),
);

/**
 * Looks up the in-depth effect text for an ability slug.
 *
 * Sources from Serebii.net's AbilityDex In-Depth Effect section.
 *
 * @param slug - The PokeAPI ability slug, e.g. "lightning-rod".
 * @returns The in-depth effect text, or null when the ability is unknown.
 */
export function getAbilityDescription(slug: string): string | null {
  return ABILITY_DESCRIPTION_BY_SLUG.get(slug) ?? null;
}

/**
 * Formats a PokeAPI ability name into a display label ("lightning-rod" ->
 * "Lightning Rod").
 *
 * @param name - The raw PokeAPI ability name.
...
/**
 * Formats a list of abilities into a comma-separated display string, e.g.
 * ["static", "lightning-rod"] -> "Static, Lightning Rod".
 *
 * @param abilities - The ability list to format.
 * @returns The joined display labels, or "" for an empty list.
 */
export function formatAbilityList(abilities: PokemonAbility[]): string {
  return abilities.map((ability) => formatAbilityName(ability.name)).join(", ");
}
